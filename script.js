(() => {
  ...
  const translations = {
    id: {
      ...
      'projects.zeflow.desc': 'Aplikasi manajemen keuangan pribadi — mencatat pemasukan, pengeluaran, dan anggaran bulanan, terhubung ke rekening bank dan e-wallet. Dirancang dan dibangun sendiri dengan Flutter.',
      ...
      'projects.zepresent.desc': 'Dashboard manajemen presensi karyawan — rekap kehadiran, jadwal shift, terintegrasi perangkat fingerprint.',
      ...
      'projects.adanest.desc': 'Website Mendukung pemulihan diri dari kecancuan PMO, Menawarkan pemantauan kebiasaan, Artikel Motivasi, Konsultasi, dan Chat bot. GEMASTIK 2023',
    ...
    en: {
      ...
      'projects.zeflow.category': 'Flutter & Mobile',
      'projects.zeflow.title': 'ZeFlow',
      'projects.zeflow.desc': 'A personal finance management app — tracking income, expenses, and monthly budgets, connected to bank accounts and e-wallets. Designed and built solo with Flutter.',
      ...
      'projects.zepresent.title': 'ZePresent',
      'projects.zepresent.desc': 'An employee attendance management dashboard — attendance logs, shift schedules, integrated with fingerprint hardware.',
      'projects.sirtuin.title': 'SIRTUIN',
  ...
  if (canHover) {
    ...
    // Glass panel: light follows the cursor across the display case.
    const heroFigure = document.getElementById('heroFigure');
    const heroGlass = document.getElementById('heroGlass');
    if (heroFigure && heroGlass) {
      heroFigure.addEventListener('mousemove', (e) => {
        const r = heroFigure.getBoundingClientRect();
        heroGlass.style.setProperty('--mx', `${((e.clientX - r.left) / r.width) * 100}%`);
        heroGlass.style.setProperty('--my', `${((e.clientY - r.top) / r.height) * 100}%`);
      });
    }
